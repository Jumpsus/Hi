...
/*   Updated: 2022/04/18 20:43:20 by pratanac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_map_bonus(t_var *var, int max_z)
{
	...
	while (var->posit[++i])
	{
		...
		{
			if ((j < ft_len_posit(var->posit[i]) - 1) && var->posit[i][j].y >= max_z && var->posit[i][j + 1].y >= max_z)
			{
				draw_me = fill_target(var->posit[i][j], var->posit[i][j + 1]);
				drawing(var, draw_me);
			}
			if (var->posit[i + 1] && var->posit[i][j].y >= max_z && var->posit[i + 1][j].y >= max_z)
			{
