...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/18 20:38:24 by pratanac          #+#    #+#             */
/*   Updated: 2022/04/18 20:39:50 by pratanac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_adj	get_adjust(t_posit **posit)
{
	...
	if (scale_x < scale_y)
		...
		adj.scale = scale_y;
	adj.offset_x = (-1 * prop.min_x * adj.scale) + (
			WIN_W - (fabs(prop.max_x - prop.min_x) * adj.scale)) / 2;
	adj.offset_y = (-1 * prop.min_y * adj.scale) + (
			WIN_H - (fabs(prop.max_y - prop.min_y) * adj.scale)) / 2;
	return (adj);
}
